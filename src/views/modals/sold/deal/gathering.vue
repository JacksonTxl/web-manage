<template>
  ...
  @ok="onOk">
      ...
      <st-form labelWidth="72px">
        <div :class="gathering('gathering')">
          ...
          <st-form-item label="支付方式" class="mgb-18" required>
            <a-radio-group @change="selectPay" v-decorator="[
              'pay_method',
    ...
    <template slot="footer">
      <st-button type="primary" @click="onSubmit">确认收款</st-button>
    </template>
  </st-modal>
</template>
...
export default {
  ...
  created() {
    this.gatheringService.getPaymentInfo('11111').subscribe()
  },
  methods: {
    onOk() {
      this.$emit('ok')
    ...
    onSubmit(e) {
      ...
      // this.gatheringService.payOrderTransaction('11111').subscribe()
    }
  }
}
</script>
