<template>
  ...
  wrapClassName="modal-shop-finance-refund">
        ...
        <div :class="refund('refund')">
          <st-form-item label="退款原因" class="mgb-18" required>
            <a-radio-group v-model="refundReason">
              <a-radio
              v-for="(item,index) in Object.keys(finance.refund_reason.value)"
              :key="index"
              :value="+item">{{finance.refund_reason.value[item]}}</a-radio>
            </a-radio-group>
          </st-form-item>
    ...
    <template slot="footer">
      ...
      <st-button @click="onSubmit" :loading="loading.orderRefund" type="primary"> 确认提交</st-button>
    </template>
  </st-modal>
</template>
