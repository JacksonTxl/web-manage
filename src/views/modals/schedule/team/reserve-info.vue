      ...
      <a-col :lg="8">
        <st-info>
          <st-info-item label="上课场地">
            {{info.court_name}}
            <span v-if="info.court_site_name">• {{info.court_site_name}}</span>
          </st-info-item>
      ...
      <thead>
        <tr>
          <th v-for="col in columns" :key="col.dataIndex">
            {{col.title}}
            <st-help-tooltip v-if="col.dataIndex == 'site_num_list'" id="TSGC002" />
          </th>
        </tr>
      </thead>
      <tbody v-if="isAdd">
          ...
          <td class="st-form-table__add">
            ...
            >
              <a-select-option
                ...
                :key="member.id"
              >{{member.member_name}}</a-select-option>
            </a-select>
          </td>
          <td>
            <a-select
          ...
          <td>
            ...
            >
              <a-select-option
                ...
                :key="siteNum.id"
              >{{siteNum.name}}</a-select-option>
            </a-select>
          </td>
          <td>
            <span slot="current_reservation_num">{{currentReservationNum}}人</span>
          </td>
          <td>未签到</td>
          <td>
            <a
