...
export default {
  ...
  methods: {
    ...
    onChange(value) {
      this.scheduleService.getScheduleTeamConsume({ course_id: this.courseId, member_id: value }).subscribe()
    },
    ...
    onClickReserve() {
      ...
      this.scheduleService.postScheduleTeamShopReserve(form).subscribe()
    },
    edit(key) {
  ...
  mounted() {
    ...
    ss.getScheduleTeamById(this.id).pipe(
      switchMap(state => {
