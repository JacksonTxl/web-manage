  ...
  wrapClassName="modal-front-add-work-note">
    <st-form :form="form" labelWidth="66px">
        <st-form-item label="待办主题" required>
            <a-input
            v-decorator="['subject',{rules:[{required: true,message:'请输入待办主题'}]}]"
    ...
    <template slot="footer">
      ...
      <st-button @click="onSubmit" :loading="loading.addWorkNote" type="primary">添加</st-button>
    </template>
  </st-modal>
</template>
<script>
  ...
  methods: {
    onSubmit() {
      this.form.validateFields((error, values) => {
        if (!error) {
          this.addWorkNoteService.addWorkNote({
