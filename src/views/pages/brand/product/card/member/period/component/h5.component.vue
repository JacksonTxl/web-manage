  ...
  <div :class="h5()">
    ...
    <div :class="other()" v-if="cardType === 0 || cardType === 1">
      <div :class="other('top')">
        ...
        <div :class="other('top__right')" v-if="data.admission_shop_list.length>0">查看全部{{`(${data.admission_shop_list.length})`}}</div>
      </div>
      <div v-if="data.admission_range.id!==2" :class="other('item')">
        {{data.admission_range.name}}
      </div>
      <div :class="other('item')" v-for="(item,index) in data.admission_shop_list.slice(0,2)" :key="index">
        {{item.shop_name}}
      </div>
    </div>
    <div :class="other()" v-if="cardType === 2">
      <div :class="other('top')">
        <div :class="other('top__title')"><span :class="other('top__icon')"></span>支持入场场馆</div>
        <div :class="other('top__right')" v-if="data.can_use_shop.length>0">查看全部{{`(${data.can_use_shop.length})`}}</div>
      </div>
      <div v-if="data.consumption_range.id!==2" :class="other('item')">
        {{data.consumption_range.name}}
      </div>
      <div :class="other('item')" v-for="(item,index) in data.can_use_shop.slice(0,2)" :key="index">
        {{item.shop_name}}
      </div>
    </div>
    <div :class="other()" v-if="cardType === 0 || cardType === 1">
    ...
    <!-- <div :class="other()">
      <div :class="other('top')">
        <div :class="other('top__title')"><span :class="other('top__icon')"></span>支持入场场馆</div>
        <div :class="other('top__right')">查看全部</div>
      </div>
      <div :class="other('item')">
        场馆名称1
      </div>
