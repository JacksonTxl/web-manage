  ...
  <st-panel app>
    ...
    >
      <div slot="action" slot-scope="record">
        <modal-link tag="a" :to="{ name: 'brand-setting-app-skillful-edit',
          props: { id: record.id, setting_name: record.setting_name }, on: { change: onListChange } }">编辑
        </modal-link>
        <a-popconfirm :title="`删除后不可进行恢复，${record.used_number ? '已标记的员工将删除此擅长项目，' : ''}确定删除此擅长项目？`" @confirm="onDelete(record.id)">
          <a class="mg-l8">删除</a>
        </a-popconfirm>
      </div>
    </st-table>
  ...
  methods: {
    ...
    onListChange() {
      if (this.page.current_page !== 1) {
        ...
        this.listService.refreshSkillfulList({
          page: 1,
          size: 20
        })
      }
