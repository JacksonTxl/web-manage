    ...
    return {
      ...
      customButtons: {
        custom1: {
          text: '设置私教排期',
          click() {
            that.$router.push({ name: 'shop-product-course-schedule-personal-personal-table', query: that.timeRange })
          }
        },
        custom2: {
          text: '复制排期',
        ...
        custom3: {
          ...
          click: () => {
            ...
            that.$nextTick().then(() => {
              that.$router.push({ name: 'shop-product-course-schedule-personal-personal-reserve-table', query: that.timeRange })
            })
          }
        },
        custom4: {
        ...
        dayHeaderEle.each(function() {
          ...
          const getDayEl = new Vue({
            ...
            render(h) {
              ...
              return (
                <GetDay onScan={this.clickHandler} title={'查看日预约'} date={dataDate} isGet={!(length === 1)}>
                </GetDay>
              )
            }
          }).$mount().$el
