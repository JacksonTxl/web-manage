<template>
  <div class="page-team-table schedule-table">
    <div class="page-team-table__title pd-x24 pd-y16 schedule-table__title" slot="title">
      <a-row :gutter="8">
        <a-col :lg="8">
          <st-button v-modal-link="{ name: 'schedule-team-add-course-schedule-batch' }" class="mg-r8" type="primary">
            批量排期
          </st-button>
...
export default {
  ...
  filters: {
    filterStartTime(val) {
      const weekday = moment(val).format('ddd').valueOf()
  ...
  methods: {
    onClickSkipSchedule() {
      this.$router.push({ name: 'shop-product-course-schedule-team', query: this.query })
    },
    getTable(val = {}) {
