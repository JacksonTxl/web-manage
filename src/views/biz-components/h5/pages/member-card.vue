<template>
  <div :class="h5()">
    <div :class="card()">
      ...
      <div :class="card('sub')" v-if="cardType === MEMBER_CARD.PERIOD_CARD">
        在可用期限内不限次数，畅快健身
      </div>
      <div :class="card('sub')" v-if="cardType === MEMBER_CARD.NUMBER_CARD">
        在可用期限内按次扣费，灵活实惠
      </div>
      <div :class="card('sub')" v-if="cardType === MEMBER_CARD.DEPOSIT_CARD">
        预约购课更方便更优惠
      </div>
    </div>
    <div :class="select()">
