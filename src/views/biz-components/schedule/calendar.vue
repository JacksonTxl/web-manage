  ...
  <div :class="bSchedule()">
    <div :class="bSchedule('toolbar')" class="pd-x24">
      ...
      <div :class="bToolbar('center')">
        <date-component :start="start" @pre="oChangeDate" @next="oChangeDate" @today="oChangeDate"></date-component>
      </div>
    ...
    <div :class="bSchedule('content')">
      ...
      <ul :class="bContent('day-group')" v-if="weeks.length === 1">
        <li class="day" :class="item | currentDay" v-for="(item, index) in weeks" :key="item.week">
          <ul class="time-group">
            ...
            <schedule-unit
              :class="{'first-unit': index===0, 'unit-day': weeks.length === 1}"
              v-for="i in 24"
              :key="i"
            >
      ...
      <ul :class="bContent('day-group')" v-else>
        <li class="day" :class="item | currentDay" v-for="(item, index) in weeks" :key="item.week">
          ...
          <schedule-card @detail="onDetail" :week="item.week" :cardList="cardList" :start="start"></schedule-card>
        </li>
      </ul>
    </div>
  </div>
